...
function synth(text: string, lang: string, gender: string, rate: string): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const p = spawn("python3", ["scripts/speak_male.py", lang, gender, rate], { cwd: process.cwd() });
    const chunks: Buffer[] = [];
    const err: Buffer[] = [];
...
export const speakMale = createServerFn({ method: "POST" })
  .validator((input: { text: string; lang?: string; gender?: string; rate?: string }) => input)
  .handler(async ({ data }) => {
    ...
    try {
      const buf = await synth(text, lang, gender, rate);
